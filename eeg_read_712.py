...
def print_data(sample):
    ...
    # 处理 channel 6 的数据
    channel_6 = sample.channels_data[5] * adc_to_microvolts  # 通道索引从 0 开始，因此 channel 6 是索引 5
    ...
    for i, value in enumerate(sample.channels_data):
        baseline_values[i].append(value * adc_to_microvolts)
...
def update_baseline_labels():
    ...
    for i in range(8):
        if len(baseline_values[i]) > 0:
            ...
            baseline_labels[i].config(text=f"Channel {i+1} Baseline: {avg_value:.2f} µV")

def start_board_stream(board):
    board.start_stream(print_data)

def set_channel_gain(board, channel, gain):
    # 构建增益设置命令
    gain_command = f"x{channel}{gain}"
    board.ser.write(gain_command.encode())
...
def start_stream():
    ...
    print("Setting channel gains...")
    for channel in range(1, 9):
        set_channel_gain(board, channel, 6)  # 设置所有通道的增益为 x24
